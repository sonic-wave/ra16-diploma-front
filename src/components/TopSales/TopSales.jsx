            ...
            <div className="row">
                {sales && sales.map(sale => (
                    <div className="col-4" key={sale.id}>
                        <div className="card">
                            <img src={sale.images[0]}
                                className="card-img-top img-fluid" alt={sale.title} />
                            <div className="card-body">
                                <p className="card-text">{sale.title}</p>
                                <p className="card-text">{formatNumber(sale.price)} руб.</p>
                                <a className="btn btn-outline-primary" onClick={() => orderClickHandler(sale.id)}>Заказать</a>
                            </div>
                        </div>
                    </div>
                ))}
